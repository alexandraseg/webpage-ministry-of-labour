...
<div class="wrapper bgded overlay gradient"  style="background-image:url('../images/demo/backgrounds/contact.jpg');">
  ...
  <h6 class="heading">Κλείστε Ραντεβού</h6>
    
    <ul>
        <li><a href="index.php">Αρχική</a></li>
      <li><a href="contact.php">Επικοινωνία</a></li>
      <li><p class="active-breadcrumb">Κλείστε Ραντεβού</p></li>
    </ul>
        ...
        <div class="login-form">
            <?php
                if ( (isset($_POST['appointment'])) ) 
                {
                    ...
                    $sql_check = mysqli_query($db,"SELECT * FROM appointments WHERE date='$date' and time='$time'");

                    $numrows = mysqli_num_rows($sql_check);
                    if ($numrows > 0)
                    {   echo '<h4 style="color:   red;">Η ημέρα και η ώρα που επιλέξατε είναι δεσμευμένη , επιλέξτε άλλη ώρα!</h4> <br>';
                    }
                    else{
                        if ((isset($_SESSION["login"]) && $_SESSION["login"] == "OK")){ 
                            $afm = $_SESSION['afm'];

                            $result = mysqli_query($db,"SELECT * FROM epixeirisi WHERE afm='$afm'");
                            $row = $result->fetch_row();
                            $name = $row[1];
                            $email = $row[3];
                            $query = "INSERT INTO appointments (id ,afm, date, time , email , name) VALUES('$afm', '$date', '$time', '$email' , '$name')";
                            ...
                            $query = "INSERT INTO appointments (afm, date, time , email , name) VALUES('$afm', '$date', '$time', '$email' , '$name')";
                            $result = mysqli_query($db,$query);
                    
                        }
                        echo '<h4 style="color:   green;">Θα λάβετε ένα mail επιβεβαίωσης κράτησης του ραντεβού!</h4> <br>';
                    }
                } 
            ?>
              <form action="appointment.php" method = "post">
                    <?php if ((isset($_SESSION["login"]) && $_SESSION["login"] == "OK")){ ?>
                        <?php if ( isset($_SESSION['type']) ) {?>
                            ...
                            <?php if ($type == '1') {?>
                                ...
                                <div class="form-row">
                                    <div class="username">
                                        <label id="shop-shopname">Επωνυμία Επιχείρησης</label>
                                        <input name="name" type="text" value="<?= $row[1]?>" pattern="[a-zA-Zα-ωΑ-Ω ]+" disabled>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                                        <label id="contact-email">Email</label>
                                        <input name="email" type="email" value="<?= $row[3]?>" disabled>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                                        <label id="contact-text">Ημέρα</label>
                                        <input name="date" type="date" class="date" required   pattern="[a-zA-Zα-ωΑ-Ω ]+" data-date-split-input="true" >
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                                        <label id="contact-text">Ώρα</label>
                                        <input type="time" id="hour" name="time" min="09:00" max="14:00" required>
                                    </div>
                                </div>
                            <?php } else if ($type == '2') {?>
                                <?php 
                                ...
                                <div class="form-row">
                                    <div class="username">
                                        <label id="name">Όνομα</label>
                                        <input name="name" type="text" placeholder="<?= $row[1]?>" pattern="[a-zA-Zα-ωΑ-Ω ]+" disabled>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="username">
                                        <label id="name">Επίθετο</label>
                                        <input name="name" type="text" placeholder="<?= $row[2]?>" pattern="[a-zA-Zα-ωΑ-Ω ]+" disabled>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                                        <label id="contact-email">Email</label>
                                        <input name="email" type="email" placeholder="<?= $row[4]?>" disabled>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                                        <label id="contact-text">Ημέρα</label>
                                        <input name="text" type="date" class="date" required   pattern="[a-zA-Zα-ωΑ-Ω ]+" data-date-split-input="true" >
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                            ...
                            <?php } else { ?>
                                <div class="form-row">
                                <div class="username">
                                    <label id="contact-name">Όνομα</label>
                                    <input name="name" type="text" pattern="[a-zA-Zα-ωΑ-Ω ]+">
                                </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                                        <label id="contact-surname">Επίθετeο</label>
                                        <input name="lastname" type="text"   pattern="[a-zA-Zα-ωΑ-Ω ]+" >
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="password">
                    ...
                    <?php } else { ?>
                        <div class="form-row">
                            <div class="username">
                                <label id="contact-name">A.Φ.Μ.</label>
                                <input name="afm" type="text" pattern="[0-9]+">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="username">
                        ...
                        <div class="form-row">
                            <div class="password">
                                <label id="contact-surname">Επίθετο</label>
                                <input name="lastname" type="text"   pattern="[a-zA-Zα-ωΑ-Ω ]+" >
                            </div>
                        </div>
                        <div class="form-row">
                    ...
                    <?php } ?>
                    <div class="log-btn">
                        <button type="submit" name="appointment"> Κράτηση</button>
                    </div>
			    </form>
          </div>
                		
          <div style="position: relative; bottom: -25px; right:-230px; " >
                <a href="contact.php">Πίσω</a>
              </div>
      </div>
    </section>
