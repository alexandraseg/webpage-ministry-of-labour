...
<div class="wrapper bgded overlay gradient" style="background-image:url('../../images/demo/backgrounds/user.jpg');">
  ...
  <h6 class="heading">Ο λογασιασμός μου</h6>
...
<div class="wrapper row3">
  <section id="cta" class="hoc container clear"> 
        ...
        <div class="profile-page-content">
            <div class="profile">
                <?php
                if (isset($_SESSION['change']) && ( $_SESSION['change'] == "OK") ) 
                {

                        echo '<h4 style="color:   green;">Eπιτυχής ενημέρωση στοιχείων !</h4> <br>';
                }
                ...
                <?php 
                 $afm = $_SESSION['afm']; 
                 $resultAnastoli = mysqli_query($db,"SELECT * FROM anastoli WHERE afm='$afm'" );
                 $numrowsAnastoli = mysqli_num_rows($resultAnastoli);

                 $resultTilergasia = mysqli_query($db,"SELECT * FROM tilergasia WHERE afm='$afm'" );
                 $numrowsTilergasia = mysqli_num_rows($resultTilergasia);

                 
                 $resultAdeia = mysqli_query($db,"SELECT * FROM adeia WHERE afm='$afm'" );
                 $numrowsAdeia = mysqli_num_rows($resultAdeia);

                 if ($numrowsAnastoli > 0){
                  $rowAnastoli = $resultAnastoli->fetch_row();
                ?>
                <h2>Σε αναστολή</h2>
                   <table >
                        <tr>
                ...
                <?php
                 ...
                 if ($numrowsTilergasia > 0){
                  $rowTilergasia = $resultTilergasia->fetch_row();
                ?>
                <h2>Σε τηλεργασία</h2>
                   <table >
                        <tr>
                        ...
                        <tr>
                          <td><?= $rowTilergasia[2] ?></td>
                          <td > <?= $rowTilergasia[3] ?> </td>
                        </tr>
                ...
                <?php
                 ...
                 if ($numrowsAdeia > 0 ){
                  $rowAdeia = $resultAdeia->fetch_row();
                ?>
                <h2>Σε Άδεια</h2>
                   <table >
                        <tr>
                ...
                <h2 style="margin-top:100px;">Στοιχεία Χρήστη</h2>
                <?php  
                    $afm = $_SESSION['afm'];
                    $result = mysqli_query($db,"SELECT * FROM users WHERE afm='$afm'");
                    $row = $result->fetch_row();
                    if ($row[3] == 1){
                      $result2 = mysqli_query($db,"SELECT * FROM epixeirisi WHERE afm='$afm'");
          ...
          <div class="profile-op">
                    <p>Θέλετε να τροποποίησετε κάποια στοιχεία του λογαριασμού σας ; <a href="edit-account.php">Τροποποίηση</a></p>
                </div>
                <div class="profile-op">
